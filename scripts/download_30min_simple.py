# -*- coding: utf-8 -*-
"""
BaoStock 30分钟数据下载工具 (简化版)

直接使用 baostock API 下载2025年A股全部30分钟K线数据，
以parquet格式保存到 SimTradeLab/data/cn/stocks_30m/ 目录。

限流策略 (基于社区最佳实践调研):
  - 基础延迟: 0.08秒/请求 (平衡速度与安全性)
  - 批量暂停: 每500只暂停3秒 (避免触发IP黑名单)
  - 自适应退避: 连续错误时指数退避 (max=5秒)
  - 断线重连: 连续失败10次自动重新登录

Usage:
    python scripts/download_30min_simple.py                    # 下载全部(断点续传)
    python scripts/download_30min_simple.py --check            # 仅检查数据完整性
    python scripts/download_30min_simple.py --codes 000001.SZ,600519.SS  # 指定股票
    python scripts/download_30min_simple.py --force             # 强制覆盖已存在文件
"""
...
# 限流参数 (基于互联网调研的最优配置)
# ============================================================
BASE_DELAY = 0.08          # 基础请求间隔 (秒) - 社区推荐 0.05~0.1s
BATCH_SIZE = 500           # 每批数量
...
def download_30min_all(stock_codes=None, start_date=DEFAULT_START_DATE,
    ...
    print("登录成功")
    print(f"  限流配置: 基础延迟={BASE_DELAY}s | 批量暂停={BATCH_PAUSE}s/{BATCH_SIZE}只 | 最大退避={MAX_BACKOFF}s")
    print()
    ...
    current_delay = BASE_DELAY  # 当前动态延迟

    for i, code in enumerate(stock_codes):
        output_path = MIN30_DIR / f"{code}.parquet"
        ...
        try:
            ...
            # 调用BaoStock API，frequency="30" 表示30分钟K线
            rs = bs.query_history_k_data_plus(
                bs_code,
                BS_FIELDS,
                start_date=start_date,
                end_date=end_date,
                frequency="30",       # <-- 关键参数：30分钟
                adjustflag="2",       # 前复权
            )
...
def check_data():
    ...
    min30_files = list(MIN30_DIR.glob("*.parquet"))
    daily_files = list(STOCKS_DIR.glob("*.parquet"))

    daily_codes = {f.stem for f in daily_files if f.stem not in INDEX_CODES}
    min30_codes = {f.stem for f in min30_files}

    missing = sorted(daily_codes - min30_codes)
    extra = sorted(min30_codes - daily_codes)

    print(f"\n日线数据: {len(daily_codes)} 只股票")
    print(f"30分钟数据: {len(min30_codes)} 只股票")
    print(f"缺失: {len(missing)} 只")
    print(f"多余: {len(extra)} 只")

    if min30_files:
        total_size = sum(f.stat().st_size for f in min30_files) / (1024 * 1024)
        print(f"总大小: {total_size:.1f} MB")

        # 抽样检查数据质量
        sample_file = min30_files[0]
